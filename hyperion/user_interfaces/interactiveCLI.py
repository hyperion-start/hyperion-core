...
class ComponentMenu(object):
    def __init__(self, cc):

        self.cc = cc

        body = [urwid.Text('Groups'), urwid.Divider()]
        for g in cc.config['groups']:
            button = urwid.Button(g['name'])
            urwid.connect_signal(button, 'click', self.group_chosen, g)
            body.append(urwid.AttrMap(button, None, focus_map='reversed'))
        group_box = urwid.ListBox(body)

        self.active_comps = urwid.SimpleFocusListWalker([urwid.Text("Such empty")])

        self.title = urwid.Text(('reversed', u'Title'), align='center')
        self.MainBox = urwid.Columns([group_box, urwid.ListBox(self.active_comps)])


    def group_chosen(self, button, group):
        self.active_comps[0] = (urwid.Pile([urwid.Text("%s@%s - Status: %s " % (t['name'], t['host'], "DUMMY")) for t
                                            in group['components']]))
...
def main(cc):
    menu = ComponentMenu(cc)
    urwid.MainLoop(menu.MainBox, palette=[('reversed', 'standout', '')]).run()
